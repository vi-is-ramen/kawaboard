...
def convert_base(value: str, from_base: str, to_base: str) -> str:
    ...
    if (from_base == 'Hex' and raw.startswith(('0x', '0X'))) \
    ...
    return {'Dec': str(num), 'Hex': hex(num).upper().replace('0X', ''),
            'Bin': bin(num).replace('0b', ''), 'Oct': oct(num).replace('0o', '')}.get(to_base,
            f'Ошибка: неизвестная целевая система "{to_base}"')
...
def _tk_dialog(kind: str, default_name: str):
    ...
    if kind == 'save':
        path = filedialog.asksaveasfilename(title='Сохранить доску', defaultextension='.kawadesk',
                                            filetypes=flt, initialfile=default_name)
    else:
...
def _ps_dialog(kind: str, default_name: str):
    ...
    if kind == 'save':
        ps = (head + "$d = New-Object System.Windows.Forms.SaveFileDialog;"  # pyright: ignore[reportImplicitStringConcatenation]
              "$d.Filter = 'Доска kawaboard (*.kawadesk)|*.kawadesk|Все файлы (*.*)|*.*';"
              "$d.DefaultExt = 'kawadesk';" + f"$d.FileName = '{default_name}';" +
              "if ($d.ShowDialog() -eq [System.Windows.Forms.DialogResult]::OK) { Write-Output $d.FileName }")
    else:
        ps = (head + "$d = New-Object System.Windows.Forms.OpenFileDialog;"  # pyright: ignore[reportImplicitStringConcatenation]
              "$d.Filter = 'Доска kawaboard (*.kawadesk)|*.kawadesk|Все файлы (*.*)|*.*';"
              "if ($d.ShowDialog() -eq [System.Windows.Forms.DialogResult]::OK) { Write-Output $d.FileName }")
